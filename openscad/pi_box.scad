...
//    protrodingThickness=2.2;
    // front wall
    if(head ==  1){
        translate([0,(lcdWidth/2) + ((baseBoardThickness+1)/2) ,width/2-(baseBoardThickness-1) - 2]) cube([lcdHeight+((baseBoardThickness+1)*2),baseBoardThickness+1,67.4],center=true);    
    }
...
//    ((width/2)-((protrodingThickness)/2))    
                translate([0,-(((protrodingThickness) /2)+(lcdWidth/2)),width/2-(baseBoardThickness-1)]) cube([lcdHeight+((baseBoardThickness+1)*2),protrodingThickness,67.41 ],center=true);
    }
...
//  }
    difference(){
        difference(){
            ...
            //translate([24,102,52]) rotate([90,0,0]) cylinder(d=13.5,h=20,center=true);   
            
       } 
       //tempSlot();
        translate([0,102,64-3.1]) rotate([90,0,0]) cylinder(r=1.9, h=10,center=true);//screw hole for rearendwall
...
//makeRearendWall(0);
    //protrodingThickness
    translate([0,-(((protrodingThickness) /2)+(lcdWidth/2)),width/2-(baseBoardThickness-1) - 3.25]) cube([lcdHeight+((baseBoardThickness+1)*2) ,2.02,67.4 ],center=true);
 union(){
...
//translate([-toTranslateX,-toTranslateYBottom,toTranslateZBottom + heightToScrew + 28.7]) color("blue") rotate([90,0,0]) cylinder(r=screwInnerRadius,h=depth/2+20,center=true);
     ...
     translate([0,-102,64-3.1]) rotate([90,0,0]) cylinder(r=1.9, h=10);
}
...
makeSideWall([0,1]);
    makePowerSlot();
    translate([38.4,80,26]) cube([10,32,28],center=true);
    translate([38.4,80,53]) rotate([0,90,0]) cylinder(d=13.5,h=20,center=true);  
       }
